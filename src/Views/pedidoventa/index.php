...
<div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
    <a href="/sistemas/comercial" class="flex items-center text-sm font-semibold text-gray-600 hover:text-blue-600 transition-colors">
    ...
    <div>
        <h2 class="text-2xl font-extrabold text-gray-800">Pedidos de Venta</h2>
    ...
    <div class="flex gap-2 bg-white p-1 rounded-lg border shadow-sm">
        <a href="/pedido-venta?estado=PENDIENTE" class="px-3 py-1.5 text-xs font-bold rounded-md <?= $filtroActual == 'PENDIENTE' ? 'bg-blue-600 text-white' : 'text-gray-500 hover:bg-gray-100' ?>">Pendientes</a>
        <a href="/pedido-venta?estado=C" class="px-3 py-1.5 text-xs font-bold rounded-md <?= $filtroActual == 'C' ? 'bg-green-600 text-white' : 'text-gray-500 hover:bg-gray-100' ?>">Cerradas (C)</a>
        <a href="/pedido-venta?estado=A" class="px-3 py-1.5 text-xs font-bold rounded-md <?= $filtroActual == 'A' ? 'bg-red-600 text-white' : 'text-gray-500 hover:bg-gray-100' ?>">Anuladas (A)</a>
        <a href="/pedido-venta?estado=ALL" class="px-3 py-1.5 text-xs font-bold rounded-md <?= $filtroActual == 'ALL' ? 'bg-gray-800 text-white' : 'text-gray-500 hover:bg-gray-100' ?>">Todas</a>
    </div>
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="p-4 text-center">
                        <div class="flex justify-center gap-2">
                            <a href="/pedido-venta/show/<?= $p['documento'] ?>" class="text-blue-500 hover:bg-blue-50 p-1.5 rounded-lg" title="Ver PV">
                            ...
                            <?php if ($p['estado'] != 'C' && $p['estado'] != 'A'): ?>
                                <a href="/pedido-venta/edit/<?= $p['documento'] ?>" class="text-amber-500 hover:bg-amber-50 p-1.5 rounded-lg" title="Editar PV">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
